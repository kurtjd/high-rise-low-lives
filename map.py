...
class Map:
    ...
    def _char_to_entity(self, char: str, x: int, y: int, game_entities_: game_entities.GameEntities) -> entity.Entity:
        new_tile: dict
        if char == '.':
        ...
        if char == '+':
            new_entity = door.Door(x, y, self.game_data, game_entities_)
        else:
            new_entity = tile.Tile(x, y, new_tile["Name"], new_tile["Desc"], new_tile["Blocked"],
                                   new_tile["Character"], new_tile["Color"], game_entities_)
        return new_entity

    # Reads a map from a text file.
    def read_map(self, file: str, game_entities_: game_entities.GameEntities) -> None:
        map_file = open(file)
        ...
        for y in enumerate(map_lines):
            ...
            for x in enumerate(y[1]):
                if x[1] != '\n':
                    # new_entity =
                    self._char_to_entity(x[1], x[0], y[0], game_entities_)
